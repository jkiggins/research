...
model, as well as the internal dynamics of the LIF neuron. Some aspects of this
...
the Astrocyte response pathways are similar in form to an LIF neuron's membrane
...
Astrocyte state variable \ca. The value of \ca will eventually leading to LTP or
LTD when explored in \ref{chapter:obj2}. This approach is outlined by Equations
...
\ca. In addition $\alpha_{ltd/ltp}$ can override default LTP/LTD behavior,
implementing anti-stdp, or biasing weight updates towards LTP/LTD. Note also,
...
that the LIF neuron to fires with a specific interval (matching the input), at a
...
plasticity. For each of these models, synaptic weight values of an LIF neuron
